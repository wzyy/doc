...
1. 如果您的私有仓库通过http访问，在第1步用Docker Hub官方镜象启动私有仓库，该私有仓库在您的Kubernetes集群外部。如果您的私有仓库通过https访问，需要在第3步安装Kubernetes时，通过Kubernetes启动私有仓库。
...
7. 在每个主机上启动一个镜像（公开镜像仓库）或可执行文件（连接到scs一个桶里），用于容器的ssh登录

8. 启动DomeOS SERVER 用公开镜像库里的镜像启动